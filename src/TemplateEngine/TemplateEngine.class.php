...
/**
 * Třída TemplateEngine je zodpovědná za správu šablon v aplikaci.
 */
final class TemplateEngine
{
    ...
    private function initializeTemplateMetadata(string $template): bool
    {
        ...
        static::$metadata['base_dir'] = strpos($template, '/') !== false ? dirname($template) . '/' : null;
    ...
    private function cacheCompiledTemplate(): void
    {
        ...
        if (flock($file, LOCK_SH)) {
            ...
            $parsedCode = (new TemplateParser(static::$conf))->parseCodeToPhp($code);
    ...
    /**
     * Přiřadí hodnoty k proměnným nebo přidá nové proměnné do kontextu.
     *
     ...
     * @return self Vrátí instanci třídy pro umožnění řetězení metod.
     */
     ...
     *
     * Tato metoda by měla být volána před renderováním nového prvku,
     * aby se zabránilo přenosu proměnných z předchozích renderů.
     *
     * @return self Vrátí instanci třídy pro umožnění řetězení metod.
     */
     ...
     *
     * @param string $templateName Název šablony, která se má vykreslit.
     * @return string Vykreslená šablona jako řetězec.
     * @throws \RuntimeException Pokud soubor s cache neexistuje.
     */
    public function render(string $templateName): string
    {
        ...
        extract($this->var);
        extract(array_merge($defaultVariables, get_defined_vars()));
