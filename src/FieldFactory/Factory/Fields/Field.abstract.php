...
abstract class Field
{
     ...
     */
    abstract public function prepareFormFieldStructure(): object;
     ...
     */
    abstract public function renderFormField(array $data, object $template): string;
    ...
    protected function assignExtraData(array $extraData, object $engine): void
    {
        ...
        $engine->assign('fieldSpecificAttributes', $extra);
    }
     ...
     * @return string
     */
    protected function renderComposedField(array $data, object $engine, string $template, array $sequence): string
    {
	 ...
	 * @param string $field Již vyrenderované HTML pole, které má být obaleno labelem.
	 * @return string Výsledný HTML kód s obaleným polem, nebo původní pole pokud není label zadán.
	 */
	protected function wrapWithLabel(array $data, object $engine, string $field): string
	{
        if (empty($data)) return $field;
	 ...
	 * @param object $engine Instance šablonovacího engine.
	 * @param string $field  Již vyrenderované HTML pole, ke kterému se má small přidat.
	 * @return string Výsledný HTML kód s připojeným small, nebo původní pole pokud není small zadán.
	 */
    protected function renderSmall(array $data, object $engine, string $field): string
    {
        if (empty($data)) return $field;
