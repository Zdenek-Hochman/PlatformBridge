...
class Parser extends VariableModifier
{
    ...
    protected static $tags = [
        ...
        'include' => array('({include\s+.*?})', '/{include\s+(?<file>[^.,\s}]+)(?:\.[^\s,}]+)?(?:\s*,\s*(?<params>[^}]+))?}/'),
        'function' => array(
            '({function.*?})',
     ...
     * @internal Tuto metodu používá Engine pro převod šablonového kódu na PHP kód, není potřeba ji volat ručně.
     */
    public function parseCodeToPhp(string $code): string
    {
        ...
        do {
            ...
            $code = $parsed;
        } while ($code !== $prevCode && preg_match('/{(variable|for|if|elseif|else|\/for|\/if)[^}]*}/', $code));
     ...
     * @return void
     * @internal Tuto metodu používá Engine pro zpracování HTML obsahu a převod na PHP kód, není potřeba ji volat ručně.
     */
    ...
    /**
     * Metoda processInclude zpracovává vložení šablony s předáním parametrů.
     *
     * Syntaxe: {include Template.tpl, key1 => $var1, key2 => $var2}
     *
     * @param array $matches Pole s výsledky regulárního výrazu.
     * @return string Vygenerovaný PHP kód pro include šablony.
     */
    private function processInclude(array $matches): string
    {
        $file = $matches['file'];
        $code = '';

        if (!empty($matches['params'])) {
            // Rozparsuje páry klíč => hodnota
            $pairs = preg_split('/\s*,\s*/', $matches['params']);
            foreach ($pairs as $pair) {
                if (preg_match('/(\w+)\s*=>\s*(.+)/', trim($pair), $pairMatch)) {
                    $key = $pairMatch[1];
                    $value = parent::transformHtmlVariables(trim($pairMatch[2]), false, false);
                    $code .= '$' . $key . ' = ' . $value . '; ';
                }
            }
        }

        $code .= 'require $this->getAndUpdateTemplateCache("' . $file . '");';
