...
class VariableModifier
{
    ...
    {
        // Detekce :raw modifikátoru na konci
        $isRaw = preg_match('/:raw\s*$/', $html) || preg_match('/^raw\s+/', $html);

        // Odstranění :raw z konce a raw z začátku
        $html = preg_replace('/:raw\s*$/', '', $html);
